...
import type { FormResult, MatchDetailsDto, MatchStandingsDto, StandingGroupDto, StandingRowDto } from "../types/index.js";
...
export function findStandingsAjaxUrl(html: string): string | null {
...
export function parseStandings(html: string, eventId: string, match: Pick<MatchDetailsDto, "homeTeam" | "awayTeam" | "competition"> | null = null): MatchStandingsDto {
...
function parseStandingGroups($: cheerio.CheerioAPI): StandingGroupDto[] {
  ...
  return tables.flatMap((table, index) => {
    ...
    if (headers.length > 0) {
      return headers.flatMap((header, headerIndex): StandingGroupDto[] => {
        const headerNode = $(header);
        const body = headerNode.nextAll("tbody").first();
        const rows = body.find("tr").toArray().flatMap((row): StandingRowDto[] => {
    ...
    const rows = tableNode.find("tbody tr").toArray().flatMap((row): StandingRowDto[] => {
      const parsed = parseStandingRow($, $(row));
      return parsed ? [parsed] : [];
    });
...
function parseStandingRow($: cheerio.CheerioAPI, row: NodeSet): StandingRowDto | null {
...
function cellText(row: NodeSet, selector: string): string | null {
  return safeText(row.find(selector).first());
}

function parseForm(node: NodeSet): FormResult[] {
  const text = cleanText(node.text()).toUpperCase();
  const explicit = text.match(/[WDL]/g);
  if (explicit?.length) return explicit.map((value) => value as FormResult);
  return node
    .find("[class*='form']")
    .toArray()
    .map((item): FormResult => {
      const className = "attribs" in item ? item.attribs?.class ?? "" : "";
      if (className.includes("-W") || className.includes("win")) return "W";
      if (className.includes("-D") || className.includes("draw")) return "D";
      if (className.includes("-L") || className.includes("loss")) return "L";
      return "?";
    });
}

function findRelevantGroups(
  groups: StandingGroupDto[],
  match: Pick<MatchDetailsDto, "homeTeam" | "awayTeam" | "competition">,
): StandingGroupDto[] {
  const bothTeams = groups.filter((group) =>
    hasTeam(group, match.homeTeam.name, match.homeTeam.id) &&
    hasTeam(group, match.awayTeam.name, match.awayTeam.id),
  );
  return bothTeams.length ? bothTeams : groups;
}

function hasTeam(group: StandingGroupDto, team: string, teamId?: string | null): boolean {
...
function findTeamRow(groups: StandingGroupDto[], team: string, teamId?: string | null): StandingRowDto | null {
  if (teamId) {
    const byId = groups.flatMap((group) => group.rows).filter((row) => row.teamId === teamId);
    if (byId.length > 0) return byId[0] ?? null;
  }
  const normalized = normalizeTeamName(team);
  ...
  if (best && best.score >= 0.88 && (!second || best.score - second.score > 0.08)) return best.row;
  return null;
}
