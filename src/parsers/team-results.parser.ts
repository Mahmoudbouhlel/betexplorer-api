...
import type {
  BetExplorerTeamReference,
  FormResult,
  MatchRecentResultsDto,
  PaginationDto,
  TeamIdentityDto,
  TeamMatchDto,
  TeamMatchOddsDto,
  ...
  TeamMatchScoreDto,
  TeamPosition,
  TeamRecentResultsDto,
  TeamResultsDto,
  TeamResultsSummaryDto,
  TeamTournamentDto,
  TeamTournamentFilterDto,
  TeamTournamentGroupDto,
} from "../types/index.js";
...
type TeamPageMode = "results" | "fixtures";
...
const emptyTournament: TeamTournamentDto = {
  tournamentId: null,
  seasonId: null,
  country: null,
  competition: null,
...
export function parseRecentResults(html: string, eventId: string): MatchRecentResultsDto {
...
export function parseTeamResultsPage(html: string, reference: BetExplorerTeamReference): TeamResultsDto {
  const source = parseTeamPageSource(html, reference, "results");
  return applyTeamResultsView(sourceToResults(source), {});
}

export function parseTeamFixturesSource(html: string, reference: BetExplorerTeamReference): TeamPageSource {
  return parseTeamPageSource(html, reference, "fixtures");
}

export function parseTeamResultsSource(html: string, reference: BetExplorerTeamReference): TeamPageSource {
  return parseTeamPageSource(html, reference, "results");
}

export function applyTeamResultsView(source: TeamResultsDto, options: TeamPageViewOptions): TeamResultsDto {
...
export function applyTeamFixturesView(
  source: {
    team: TeamIdentityDto;
    filters?: { availableTournaments: TeamTournamentFilterDto[] };
    matches: TeamMatchDto[];
    scrapedAt: string;
  },
...
export function sourceToResults(source: TeamPageSource): TeamResultsDto {
  return {
    team: source.team,
...
export function parseBetExplorerTeamScore(raw: string): TeamMatchScoreDto {
...
export function calculateTeamResultsSummary(matches: TeamMatchDto[], formLimit = 10): TeamResultsSummaryDto {
  ...
  const totals = finished.reduce((acc, match) => {
    ...
    if (perspective.forGoals > perspective.againstGoals) acc.wins += 1;
    if (perspective.forGoals === perspective.againstGoals) acc.draws += 1;
    if (perspective.forGoals < perspective.againstGoals) acc.losses += 1;
    if (perspective.againstGoals === 0) acc.cleanSheets += 1;
    if (perspective.forGoals === 0) acc.failedToScore += 1;
    if (perspective.forGoals > 0 && perspective.againstGoals > 0) acc.bothTeamsScored += 1;
    ...
    if (totalGoals < 2.5) acc.under25 += 1;
    if (match.selectedTeamPosition === "HOME") {
      acc.homeMatches += 1;
      if (perspective.forGoals > perspective.againstGoals) acc.homeWins += 1;
    }
    if (match.selectedTeamPosition === "AWAY") {
      acc.awayMatches += 1;
      if (perspective.forGoals > perspective.againstGoals) acc.awayWins += 1;
    }
    return acc;
  }, seed);
...
function parseTeamPageSource(html: string, reference: BetExplorerTeamReference, mode: TeamPageMode): TeamPageSource {
  const $ = cheerio.load(html);
  ...
  let currentTournament: TeamTournamentDto = emptyTournament;
  const matches: TeamMatchDto[] = [];
...
function parseTeamIdentity($: cheerio.CheerioAPI, reference: BetExplorerTeamReference): TeamIdentityDto {
...
function extractTeamNameFromBreadcrumb($: cheerio.CheerioAPI, reference: BetExplorerTeamReference): string | null {
  const records = extractJsonLdRecords($);
  for (const record of records) {
    if (stringValue(record["@type"]) !== "BreadcrumbList") continue;
    ...
    if (exactName && !/results|fixtures|summary/i.test(exactName)) return exactName;
    const fallback = items
      .map((item) => stringValue(item.name))
      .filter((value): value is string => Boolean(value && !/results|fixtures|summary|football/i.test(value)))
      .at(-1);
...
function extractJsonLdRecords($: cheerio.CheerioAPI): JsonRecord[] {
  const records: JsonRecord[] = [];
  $('script[type="application/ld+json"]').each((_index, element) => {
    const raw = cleanText($(element).text());
    if (!raw) return;
    try {
      collectJsonRecords(JSON.parse(raw) as unknown, records);
    } catch {
      return;
    }
  });
...
function extractTeamNameFromMeta($: cheerio.CheerioAPI): string | null {
  const candidates = [
  ...
  for (const candidate of candidates) {
    const cleaned = cleanText(candidate)
      ...
      .replace(/\s+/g, " ")
      .trim();
    if (cleaned && /\p{Letter}/u.test(cleaned) && !/^football$/i.test(cleaned)) return cleaned;
  }
  return null;
}

function parseTournamentFilters($: cheerio.CheerioAPI): TeamTournamentFilterDto[] {
...
function parseTournamentRow($: cheerio.CheerioAPI, row: NodeSet): TeamTournamentDto {
...
function parseTournamentLabel(label: string): { country: string | null; competition: string | null; stage: string | null } {
  const cleaned = cleanText(label);
  if (!cleaned) return { country: null, competition: null, stage: null };
...
function parseTeamMatchRow(
  $: cheerio.CheerioAPI,
  row: NodeSet,
  currentTournament: TeamTournamentDto,
  team: TeamIdentityDto,
  mode: TeamPageMode,
): TeamMatchDto | null {
...
function tournamentFromRow(row: NodeSet, currentTournament: TeamTournamentDto): TeamTournamentDto {
  const rowTournamentId = cleanText(row.attr("data-ttid"));
  if (!rowTournamentId || rowTournamentId === currentTournament.tournamentId) return currentTournament;
  return {
    ...currentTournament,
...
function findScoreCell(cells: NodeSet[]): NodeSet | null {
  const byColspan = cells.find((cell) => {
    const text = cleanText(cell.text());
    return cell.attr("colspan") === "3" && Boolean(text);
  });
  if (byColspan) return byColspan;
  return (
    cells.find((cell) => {
      const text = cleanText(cell.text());
...
function isTeamCell(cell: NodeSet, scoreCell: NodeSet | null): boolean {
  if (scoreCell && sameNode(cell, scoreCell)) return false;
  if (cell.is(".table-main__formicon") || cell.find(selectors.teamPage.formIcon).length > 0) return false;
  if (cell.is(selectors.teamPage.dateCell)) return false;
  if (findDetailsUrl(cell)) return false;
  const text = cleanText(cell.text());
  if (!text || /^details$/i.test(text)) return false;
  if (cell.find('a[href*="/football/team/"]').length > 0) return true;
  if (cell.find("strong").length > 0) return true;
  return /\p{Letter}/u.test(text);
}

function sameNode(left: NodeSet, right: NodeSet): boolean {
  const leftNode = left.get(0);
  const rightNode = right.get(0);
  return Boolean(leftNode && rightNode && leftNode === rightNode);
}

function parseTeamCell(cell: NodeSet | undefined): { name: string; teamId: string | null; teamUrl: string | null } {
  if (!cell) return { name: "", teamId: null, teamUrl: null };
...
function selectedTeamPosition(
  team: TeamIdentityDto,
  homeTeam: { name: string; teamId: string | null },
  awayTeam: { name: string; teamId: string | null },
  homeCell: NodeSet | undefined,
  awayCell: NodeSet | undefined,
): TeamPosition {
  if (homeCell?.find("strong").length) return "HOME";
  if (awayCell?.find("strong").length) return "AWAY";
  const normalizedSelected = normalizeTeamName(team.name);
  if (normalizeTeamName(homeTeam.name) === normalizedSelected) return "HOME";
  if (normalizeTeamName(awayTeam.name) === normalizedSelected) return "AWAY";
  if (homeTeam.teamId === team.teamId) return "HOME";
  if (awayTeam.teamId === team.teamId) return "AWAY";
  return "UNKNOWN";
}

function parseFormIcon(row: NodeSet): TeamMatchResult {
  const className = row.find(selectors.teamPage.formIcon).first().attr("class") ?? "";
  if (/\bicon__w\b/i.test(className)) return "W";
  if (/\bicon__d\b/i.test(className)) return "D";
  if (/\bicon__l\b/i.test(className)) return "L";
  return "UNKNOWN";
}

function deriveResultFromScore(position: TeamPosition, score: TeamMatchScoreDto): TeamMatchResult {
  if (position === "UNKNOWN" || score.home === null || score.away === null) return "UNKNOWN";
...
function inferStatus(raw: string | null, score: TeamMatchScoreDto, mode: TeamPageMode): TeamMatchDto["status"] {
  const text = cleanText(raw).toLowerCase();
  if (/postponed|postp|pstp/.test(text)) return "POSTPONED";
  if (/cancelled|canceled|aband/.test(text)) return "CANCELLED";
  if (mode === "fixtures") return "SCHEDULED";
  if (score.home !== null && score.away !== null) return "FINISHED";
  if (/^\d{1,2}:\d{2}$/.test(text)) return "SCHEDULED";
  return "UNKNOWN";
}

function parseTeamOdds(row: NodeSet): TeamMatchOddsDto {
...
function findDetailsUrl(scope: NodeSet): string | null {
  ...
  for (const anchor of anchors) {
    ...
    if (!href) continue;
    if (text === "details") return href;
    if (isFootballMatchEventUrl(href)) return href;
  }
  return null;
}

function isFootballMatchEventUrl(url: string): boolean {
  const absolute = absoluteBetExplorerUrl(url);
  if (!absolute) return false;
  const parsed = new URL(absolute);
  const segments = parsed.pathname.split("/").filter(Boolean);
  if (segments.length < 5) return false;
  if (segments[0] !== "football") return false;
  if (segments[1] === "team") return false;
  return extractEventIdFromUrl(absolute) !== null;
}

function emptyScore(raw: string | null): TeamMatchScoreDto {
  return {
    home: null,
    away: null,
    halfTimeHome: null,
...
function scorePair(match: RegExpMatchArray | undefined): { home: number | null; away: number | null } {
...
function groupByTournament(matches: TeamMatchDto[]): TeamTournamentGroupDto[] {
  const groups = new Map<string, TeamTournamentGroupDto>();
...
function paginate(matches: TeamMatchDto[], page = 1, limit = 20): PaginationDto {
...
function normalizedTournamentFilter(source: TeamResultsDto, tournamentId: string | null): string | null {
  ...
  if (!availableIds.has(tournamentId) && !matchHasTournament) throw new Error("TEAM_TOURNAMENT_FILTER_INVALID");
  return tournamentId;
}

function normalizedFixturesTournamentFilter(filters: TeamTournamentFilterDto[], tournamentId: string | null): string | null {
  if (!tournamentId) return null;
...
function normalizeFormLimit(value: number): number {
  const allowed = new Set([5, 10, 15, 20, 50]);
  return allowed.has(value) ? value : 10;
}

function isSummarizable(match: TeamMatchDto): boolean {
  return match.status === "FINISHED" && selectedPerspectiveScore(match) !== null;
}

function selectedPerspectiveScore(match: TeamMatchDto): { forGoals: number; againstGoals: number } | null {
  if (match.score.home === null || match.score.away === null) return null;
  if (match.selectedTeamPosition === "HOME") return { forGoals: match.score.home, againstGoals: match.score.away };
  if (match.selectedTeamPosition === "AWAY") return { forGoals: match.score.away, againstGoals: match.score.home };
  return null;
}
...
function averageValue(value: number, total: number): number | null {
  if (total === 0) return null;
...
function isLegitimateNoMatchesPage($: cheerio.CheerioAPI): boolean {
  const pageText = cleanText($.root().text()).toLowerCase();
  return /no\s+(matches|results|fixtures)|there are no|no data/.test(pageText);
}

function hasResultsLanguage($: cheerio.CheerioAPI, mode: TeamPageMode): boolean {
  const title = cleanText($(selectors.teamPage.title).first().text()).toLowerCase();
  if (mode === "fixtures") return /fixtures|matches/.test(title);
  return /results|fixtures|matches/.test(title);
}

function hasMatchLikeRows(table: NodeSet): boolean {
...
function parseTeamSection($: cheerio.CheerioAPI, section: NodeSet, scrapedAt: string, fallbackTeam?: string | null): TeamRecentResultsDto {
...
function parseFormForRow($: cheerio.CheerioAPI, row: NodeSet, team: string): FormResult {
  const formClass = row.find(selectors.recentResults.form).attr("class") ?? "";
  if (formClass.includes("last-results__form-results-W")) return "W";
  if (formClass.includes("last-results__form-results-D")) return "D";
  if (formClass.includes("last-results__form-results-L")) return "L";
