...
import type { BookmakerOddsDto, MatchDetailsDto, MatchOddsDto, MatchStatus } from "../types/index.js";
...
interface ParsedDetailOdds {
  best: MatchOddsDto | null;
  reference: MatchOddsDto | null;
  rows: BookmakerOddsDto[];
}

export function parseMatchDetails(html: string, url: string, eventId = extractEventIdFromUrl(url) ?? ""): MatchDetailsDto {
...
export function findSportsEventJsonLd($: cheerio.CheerioAPI): StructuredSportsEvent | null {
...
function parseDetailOdds($: cheerio.CheerioAPI): ParsedDetailOdds {
  const rows: BookmakerOddsDto[] = [];
  $("#bestOddsComponent table.oddsComparison__table tbody tr").each((_index, element) => {
    ...
    const parsedRow: BookmakerOddsDto = {
...
function oddFromCell(cell: NodeSet | undefined): number | null {
  if (!cell) return null;
...
function maxOdd(left: number | null, right: number | null): number | null {
...
function resolveStatus(input: {
  ...
  if (visible === "FINISHED" || jsonLd === "FINISHED") return "FINISHED";
  if (visible === "POSTPONED" || jsonLd === "POSTPONED") return "POSTPONED";
  if (visible === "CANCELLED" || jsonLd === "CANCELLED") return "CANCELLED";
  if (visible === "ABANDONED") return "ABANDONED";
  if (visible === "LIVE" || jsonLd === "LIVE") return "LIVE";
  if (hasCompleteScore && /(?:\(\d+\s*[:–—-]\s*\d+)/u.test(visibleText)) return "FINISHED";
  if (visible === "SCHEDULED" || jsonLd === "SCHEDULED") return "SCHEDULED";
  if (hasCompleteScore) return "FINISHED";
  return "UNKNOWN";
}

function normalizeVisibleStatus(status: string | null): MatchStatus | null {
  const text = cleanText(status).toUpperCase();
  if (!text) return null;
  if (/^(FIN|FT|AET|PEN)$/.test(text)) return "FINISHED";
  if (/^\d{1,2}:\d{2}$/.test(text)) return "SCHEDULED";
  if (/^\d{1,3}(?:\+\d{1,2})?'$/.test(text) || text === "HT") return "LIVE";
  if (/POSTP/.test(text)) return "POSTPONED";
  if (/CANC/.test(text)) return "CANCELLED";
  if (/ABN/.test(text)) return "ABANDONED";
  return null;
}

function normalizeJsonLdStatus(status: string | null): MatchStatus | null {
  if (!status) return null;
  if (status.includes("EventScheduled") || status.includes("EventMovedOnline")) return "SCHEDULED";
  if (status.includes("EventInProgress")) return "LIVE";
  if (status.includes("EventCompleted") || status.includes("EventEnded")) return "FINISHED";
  if (status.includes("EventPostponed")) return "POSTPONED";
  if (status.includes("EventCancelled")) return "CANCELLED";
  return null;
}

function teamName(link: NodeSet, fallback: string | null | undefined): string {
  return safeText(link.find(".list-details__item__title, .teamsLink").first()) ?? safeText(link) ?? fallback ?? "";
}

function teamLogo(link: NodeSet, fallback: string | null | undefined): string | null {
  return (
    absoluteBetExplorerUrl(link.find("img").first().attr("src")) ??
...
function breadcrumbCompetition($: cheerio.CheerioAPI): string | null {
  const items = $('script[type="application/ld+json"]')
    .toArray()
    .flatMap((script) => {
      try {
        const parsed = JSON.parse(cleanText($(script).text())) as unknown;
        const records: JsonRecord[] = [];
...
function inferSport(url: string): string | null {
