...
export function parseMatchList(
  ...
  const matches: MatchSummaryDto[] = [];
  const indexByEventId = new Map<string, number>();

  $(selectors.matchList.matchRow).each(
    (_index, element) => {
      const parsed = parseMatchRow(
        $,
        $(element),
      );
      ...
      const existingIndex =
        indexByEventId.get(parsed.eventId);
...
function parseMatchRow(
  ...
  const homeLogo = extractParticipantLogo(
    homeContainer,
  );

  const awayLogo = extractParticipantLogo(
    awayContainer,
  );

  const rawRelativeUrl =
    cleanText(matchLink.attr("href")) ||
  ...
  const relativeUrl =
    relativeBetExplorerUrl(
      eventUrl || rawRelativeUrl,
    ) ?? rawRelativeUrl ?? "";

  const rawDate =
    info.attr("data-dt") ?? null;
   ...
   */
  const scoreContainer = findScoreContainer(
    matchLink,
    info,
  );
  ...
  const oddsContainer = info
    .find(
      "li.table-main__oddsLi > " +
        ".table-main__oddsLi.oddsColumn",
    )
    .first();

  const oddsNodes =
    oddsContainer.length > 0
      ? oddsContainer.find(
      ...
      : info
          ...
          .filter(
            (_index, element) =>
              !$(element).closest(
                '[data-live-cell="score"], .mainResult',
              ).length,
          );

  const context = findLeagueContext(
    $,
    row,
  );
  ...
  const rawTime =
    safeText(
      info
        .find(
          ".table-main__matchStatus[data-live-cell='time'], " +
  ...
  return {
    ...
    dateTime:
      parseBetExplorerDateTime(rawDate),

    time: rawTime,

    status: inferMatchStatus(
      row,
      rawTime,
      scoreText,
    ),
...
function findLeagueContext(
  ...
  const container = row.closest(
    selectors.matchList.leagueContainer,
  );

  const header = container
    .find(
      selectors.matchList.leagueHeader,
    )
    .first();
  ...
  const leagueText =
    safeText(
      header
        .find(
          selectors.matchList.leagueName,
        )
        .first(),
    ) ??
    safeText(header) ??
  ...
  const flag = header
    .find(
      selectors.matchList.countryFlag,
    )
    .first();

  const countryFromFlag =
...
function inferMatchStatus(
  ...
  if (
    ...
    /\bft\b/.test(time) ||
    /after penalties|aet|ended/.test(
      text,
    )
  ) {
    return "FINISHED";
  ...
  if (
    /\bht\b/.test(time) ||
    /^\d{1,3}'$/.test(time) ||
    /live|half-time|second half/.test(
      `${text} ${time}`,
    )
  ) {
    return "LIVE";
...
function getMatchCompletenessScore(
  match: MatchSummaryDto,
): number {
  let score = 0;

  if (match.homeTeam) score += 2;
  if (match.awayTeam) score += 2;

  if (match.homeLogo) score += 1;
  if (match.awayLogo) score += 1;
  ...
  if (match.country) score += 1;
  if (match.league) score += 1;
  if (match.eventUrl) score += 1;
