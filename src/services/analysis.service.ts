import type { MatchAnalysisDto, MatchDetailsDto, MatchH2HDto, MatchOddsDto, MatchRecentResultsDto, MatchStandingsDto } from "../types/index.js";
...
export async function getMatchAnalysis(
  eventId: string,
  options: { url?: string } = {},
): Promise<{ data: MatchAnalysisDto; warnings: string[] }> {
  ...
  const [standings, h2h, recentResults] = await Promise.all([
    getMatchStandings(eventId, { url: options.url ?? details.url, backgroundRefresh: false }).then((response) => response.data).catch((error: unknown) => {
      warnings.push(errorMessage(error));
      return null;
    }),
    getMatchH2H(eventId, { url: options.url ?? details.url, backgroundRefresh: false }).then((response) => response.data).catch((error: unknown) => {
      warnings.push(errorMessage(error));
      return null;
    }),
    getRecentResults(eventId, { url: options.url ?? details.url, backgroundRefresh: false }).then((response) => response.data).catch((error: unknown) => {
      warnings.push(errorMessage(error));
      return null;
    }),
  ]);
...
function analyzeStructuredMatch(
  eventId: string,
...
function marketAnalysis(odds: MatchOddsDto | null | undefined): MatchAnalysisDto["market"] {
  const raw = {
  ...
  const sum = rawValues.reduce<number>((total, value) => total + (value ?? 0), 0);
...
function candidatesFrom(probabilities: MatchAnalysisDto["probabilities"], odds: MatchOddsDto | null | undefined): MatchAnalysisDto["candidates"] {
  const markets = [
    { type: "HOME" as const, probability: probabilities.home, odd: odds?.home },
    { type: "DRAW" as const, probability: probabilities.draw, odd: odds?.draw },
    { type: "AWAY" as const, probability: probabilities.away, odd: odds?.away },
  ];
  return markets.flatMap((market) => {
    if (market.probability === null || !market.odd) return [];
    ...
    if (edge < 0.03 || expectedValue <= 0) return [];
    return [{
      type: market.type,
...
function recommendationFrom(
  mode: MatchAnalysisDto["mode"],
  candidates: MatchAnalysisDto["candidates"],
  dataQuality: number,
  confidence: number,
): MatchAnalysisDto["recommendation"] {
  if (mode === "POST_MATCH") {
  ...
  if (dataQuality < 70) return { decision: "NO_BET", primaryCandidate: null, reasons: ["Data quality below 70."] };
  if (confidence < 65) return { decision: "NO_BET", primaryCandidate: null, reasons: ["Confidence below 65."] };
  const primaryCandidate = candidates[0] ?? null;
  if (!primaryCandidate) return { decision: "NO_BET", primaryCandidate: null, reasons: ["No positive expected-value candidate above edge threshold."] };
...
function analysisMode(status: MatchDetailsDto["status"]): MatchAnalysisDto["mode"] {
...
function actualResultFrom(details: MatchDetailsDto): MatchAnalysisDto["actualResult"] {
  if (details.homeScore === null || details.awayScore === null) return null;
  return {
...
function missingInputs(
  ...
  if (!details.odds?.available) missing.push("odds");
  if (!standings?.groups.length) missing.push("standings");
  if (!h2h?.matches.length) missing.push("h2h");
  if (recentSampleSize(recentResults) === 0) missing.push("recent results");
  return missing;
}

function recentSampleSize(recentResults: MatchRecentResultsDto | null): number {
...
function supportingFactors(
  ...
  if (standings?.homeTeamRow && standings.awayTeamRow) factors.push("Current teams resolved in standings.");
  if (h2h?.matches.length) factors.push("Head-to-head rows parsed; current event is excluded from pre-match sample factors.");
  if (recentSampleSize(recentResults) > 0) factors.push("Recent-result sample parsed from source.");
  return factors;
}
...
function qualityLevel(score: number): MatchAnalysisDto["dataQuality"]["level"] {
...
function withoutUndefinedUrl(url: string | undefined): { url?: string; backgroundRefresh: false } {
