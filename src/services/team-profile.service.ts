...
import { getTeamResults, resolveTeamReference, type TeamResultsRequestOptions } from "./team-results.service.js";
...
export async function getTeamProfile(teamId: string, options: TeamProfileRequestOptions = {}): Promise<CachedResponse<TeamProfileDto>> {
  ...
  if (entry && !force) {
    ...
    if (stale && backgroundRefresh) void refreshTeamProfile(teamId, options).catch(() => undefined);
    return {
      data: entry.data,
...
export async function refreshTeamProfile(teamId: string, options: TeamProfileRequestOptions = {}): Promise<TeamProfileDto> {
  validateTeamId(teamId);
  const reference = resolveTeamReference(teamId, options);
  const key = teamProfileCacheKey(teamId);
...
export async function getMatchTeams(
  eventId: string,
  options: { url?: string; force?: boolean; backgroundRefresh?: boolean } = {},
  ...
  if (entry && !force) {
    const stale = Date.now() - entry.updatedAt > cacheTtl.teamProfile * 1000;
    if (stale && backgroundRefresh) void refreshMatchTeams(eventId, options.url).catch(() => undefined);
    return {
      data: entry.data,
      cache: createCacheMetadata(entry, cacheTtl.teamProfile, stale && backgroundRefresh, true),
...
export async function refreshMatchTeams(eventId: string, url?: string): Promise<MatchTeamsDto> {
  validateEventId(eventId);
...
export function parseMatchTeamsFromHtml(html: string, eventId: string, matchUrl: string): MatchTeamsDto {
  validateEventId(eventId);
  ...
  const away =
    findTeamReference(
      candidates.filter((candidate) => candidate.teamId !== home?.teamId),
...
function uniqueTeamReferences($: cheerio.CheerioAPI): TeamReferenceDto[] {
  const seen = new Set<string>();
  const references: TeamReferenceDto[] = [];
  $('a[href*="/football/team/"]').each((_index, element) => {
    const href = $(element).attr("href");
    if (!href) return;
    let reference: BetExplorerTeamReference;
    try {
      reference = parseTeamReferenceFromUrl(new URL(href, "https://www.betexplorer.com").toString());
    } catch {
      return;
    }
    if (seen.has(reference.teamId)) return;
...
function findTeamReference(candidates: TeamReferenceDto[], name: string): TeamReferenceDto | null {
  const normalized = normalizeTeamName(name);
  if (!normalized) return null;
  return candidates.find((candidate) => normalizeTeamName(candidate.name) === normalized) ?? null;
}

function teamSectionOptions(reference: BetExplorerTeamReference, options: TeamProfileRequestOptions, limit: number): TeamResultsRequestOptions {
  const sectionOptions: TeamResultsRequestOptions = {
    slug: reference.slug,
    page: 1,
    limit,
  };
  if (options.backgroundRefresh !== undefined) sectionOptions.backgroundRefresh = options.backgroundRefresh;
  if (options.force !== undefined) sectionOptions.force = options.force;
  return sectionOptions;
}

function fallbackTeamIdentity(reference: BetExplorerTeamReference): TeamIdentityDto {
  return {
    teamId: reference.teamId,
    slug: reference.slug,
...
function teamProfileCacheKey(teamId: string): string {
  return `profile:${validateTeamId(teamId)}`;
}

function matchTeamsCacheKey(eventId: string): string {
