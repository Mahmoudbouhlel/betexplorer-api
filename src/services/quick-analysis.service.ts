...
import type {
  ...
  MatchRecommendationMarket,
  MatchStatus,
  MatchStandingsDto,
  MatchSummaryDto,
  QuickMatchAnalysisDto,
...
interface Quality {
  dataQuality: { score: number; label: AnalysisLabel };
  confidence: { score: number; label: AnalysisLabel };
  risk: AnalysisRisk;
}

const marketLabels: Record<MatchRecommendationMarket, string> = {
  HOME: "1",
  DRAW: "X",
  AWAY: "2",
  HOME_OR_DRAW: "1X",
...
const marketPriority: MatchRecommendationMarket[] = [
  "HOME",
  "DRAW",
  "AWAY",
  "HOME_OR_DRAW",
...
export async function getQuickMatchAnalyses(eventIds: string[]): Promise<QuickMatchAnalysisDto[]> {
...
export async function getQuickMatchAnalysis(eventId: string): Promise<QuickMatchAnalysisDto> {
...
async function loadQuickInputs(eventId: string): Promise<QuickInputs> {
  ...
  let loadedDetails: MatchDetailsDto;
  try {
  ...
  const [standings, h2h, recentResults] = await Promise.all([
    getMatchStandings(eventId, { url, backgroundRefresh: false }).then((response) => response.data).catch((error: unknown) => {
      inputWarnings.push(errorMessage(error));
      return null;
    }),
    getMatchH2H(eventId, { url, backgroundRefresh: false }).then((response) => response.data).catch((error: unknown) => {
      inputWarnings.push(errorMessage(error));
      return null;
    }),
    getRecentResults(eventId, { url, backgroundRefresh: false }).then((response) => response.data).catch((error: unknown) => {
      inputWarnings.push(errorMessage(error));
      return null;
    }),
  ]);
...
function buildQuickAnalysis(eventId: string, inputs: QuickInputs): QuickMatchAnalysisDto {
...
function resolvedStatus(inputs: QuickInputs): MatchStatus {
  const detailStatus = inputs.details?.status;
  if (detailStatus && detailStatus !== "UNKNOWN") return detailStatus;
  return inputs.summary?.status ?? detailStatus ?? "UNKNOWN";
}

function analysisMode(status: MatchStatus): AnalysisMode {
...
function normalize1x2(odds: MatchOddsDto | null): Pick<QuickMatchAnalysisDto["probabilities"], "home" | "draw" | "away"> {
  const raw = {
...
function formFrom(recentResults: MatchRecentResultsDto | null): QuickMatchAnalysisDto["form"] {
  return {
...
function statsFrom(eventId: string, recentResults: MatchRecentResultsDto | null, h2h: MatchH2HDto | null): CompactStats {
...
function expectedGoalsFrom(stats: CompactStats): QuickMatchAnalysisDto["expectedGoals"] {
...
function goalProbabilitiesFrom(
  stats: CompactStats,
  expectedGoals: QuickMatchAnalysisDto["expectedGoals"],
): Omit<QuickMatchAnalysisDto["probabilities"], "home" | "draw" | "away"> {
...
function recommendationsFrom(
  probabilities: QuickMatchAnalysisDto["probabilities"],
  odds: MatchOddsDto | null,
  quality: Quality,
  mode: AnalysisMode,
  form: QuickMatchAnalysisDto["form"],
  ...
  const recommendations: QuickRecommendationDto[] = [
...
function outcomeRecommendation(
...
function statisticalRecommendation(
...
function alternativesFrom(primary: QuickRecommendationDto | null, recommendations: QuickRecommendationDto[]): QuickRecommendationDto[] {
  if (!primary) return [];
  const alternatives: QuickRecommendationDto[] = [];
  for (const recommendation of recommendations) {
    if (!recommendation.eligible || recommendation.market === primary.market) continue;
    if (isContradictory(primary.market, recommendation.market)) continue;
    if (alternatives.some((item) => isContradictory(item.market, recommendation.market))) continue;
...
function decisionFrom(mode: AnalysisMode, primary: QuickRecommendationDto | null): RecommendationDecision {
  if (mode === "POST_MATCH") return "POST_MATCH_ONLY";
  if (mode === "LIVE" || mode === "UNAVAILABLE") return "UNAVAILABLE";
  return primary ? "BET_CANDIDATE" : "NO_BET";
}

function rejectionReasonsFrom(
  decision: RecommendationDecision,
  quality: Quality,
  odds: MatchOddsDto | null,
  form: QuickMatchAnalysisDto["form"],
  contradictions: string[],
  primary: QuickRecommendationDto | null,
  inputs: QuickInputs,
): string[] {
  if (decision === "BET_CANDIDATE" || primary) return [];
  if (decision === "POST_MATCH_ONLY") return ["Finished match; active recommendations are disabled."];
  if (decision === "UNAVAILABLE") return ["Match state is not suitable for pre-match betting analysis."];

  const reasons: string[] = [];
  if (form.homeSampleSize + form.awaySampleSize < 4) reasons.push("Insufficient recent-match data");
  if (quality.dataQuality.score < 60) reasons.push("Low data quality");
  if (quality.confidence.score < 55) reasons.push("Low confidence");
  if (!completeOdds(odds)) reasons.push("Incomplete odds");
  if (contradictions.length) reasons.push("Conflicting signals");
  if (completeOdds(odds)) reasons.push("No positive market edge");
  for (const warning of inputs.inputWarnings.slice(0, 2)) reasons.push(warning);
  return uniqueStrings(reasons);
}

function supportingFactorsFrom(
  probabilities: QuickMatchAnalysisDto["probabilities"],
  odds: MatchOddsDto | null,
  form: QuickMatchAnalysisDto["form"],
  stats: CompactStats,
  ...
  if (primary?.eligible) factors.push(`${primary.label} passed backend eligibility checks with ${formatPercent(primary.probability)} model probability.`);
  if (completeOdds(odds)) factors.push("Complete 1X2 market odds were available for probability normalization.");
  if (form.homeSampleSize || form.awaySampleSize) factors.push(`Recent-result samples parsed: home ${form.homeSampleSize}, away ${form.awaySampleSize}.`);
  if (expectedGoals.total !== null) factors.push(`Estimated combined expected goals are ${expectedGoals.total}.`);
  if (probabilities.over15 !== null) factors.push(`Over 1.5 model probability is ${formatPercent(probabilities.over15)}.`);
  if (probabilities.bttsYes !== null) factors.push(`GG Yes model probability is ${formatPercent(probabilities.bttsYes)}.`);
  if (stats.h2hMatches > 0) factors.push(`H2H sample includes ${stats.h2hMatches} completed non-current match${stats.h2hMatches === 1 ? "" : "es"}.`);
  if (inputs.standings?.homeTeamRow && inputs.standings.awayTeamRow) factors.push("Both teams were resolved in the standings table.");
  return uniqueStrings(factors);
}

function contradictionsFrom(
  probabilities: QuickMatchAnalysisDto["probabilities"],
  odds: MatchOddsDto | null,
  stats: CompactStats,
  inputs: QuickInputs,
  ...
  if (recentSamples > 0 && recentSamples < 4) contradictions.push("Recent sample size is small.");
  if (!completeOdds(odds)) contradictions.push("The 1X2 market is incomplete.");
  if (quality.dataQuality.score < 60) contradictions.push("Data quality is below the smart-card recommendation threshold.");
  if (stats.h2hOver25Rate !== null && averageDefined([stats.homeOver25Rate, stats.awayOver25Rate]) !== null) {
    ...
    if (Math.abs(recentOver25 - stats.h2hOver25Rate) >= 0.35) contradictions.push("H2H goal trend disagrees with recent-form goal trend.");
  }
  if (probabilities.home !== null && probabilities.draw !== null && probabilities.away !== null) {
    ...
    if (spread < 0.15) contradictions.push("Outcome probabilities are closely grouped.");
  }
  for (const warning of inputs.inputWarnings.slice(0, 2)) contradictions.push(warning);
  return uniqueStrings(contradictions);
}

function unavailableQuickAnalysis(eventId: string, reason: string): QuickMatchAnalysisDto {
  return {
    eventId,
    status: "UNKNOWN",
    mode: "UNAVAILABLE",
...
function emptyProbabilities(): QuickMatchAnalysisDto["probabilities"] {
  return {
    home: null,
    draw: null,
    away: null,
...
function completeOdds(odds: MatchOddsDto | null): boolean {
  return Boolean(odds?.available && odds.home && odds.draw && odds.away);
}

function summarizableMatches(matches: HistoricalMatchDto[]): HistoricalMatchDto[] {
  return matches.filter((match) => match.homeScore !== null && match.awayScore !== null);
}

function totalGoals(match: HistoricalMatchDto): number {
  return (match.homeScore ?? 0) + (match.awayScore ?? 0);
}

function bothTeamsScored(match: HistoricalMatchDto): boolean {
  return (match.homeScore ?? 0) > 0 && (match.awayScore ?? 0) > 0;
}

function rate(matches: HistoricalMatchDto[], predicate: (match: HistoricalMatchDto) => boolean): number | null {
  if (!matches.length) return null;
  return roundProbability(matches.filter(predicate).length / matches.length);
}

function averageFromTotal(total: number | undefined, count: number): number | null {
  if (!count || typeof total !== "number" || !Number.isFinite(total)) return null;
...
function poissonOver(lambda: number, line: 1.5 | 2.5): number {
...
function implied(odd: number | null | undefined): number | null {
  return typeof odd === "number" && Number.isFinite(odd) && odd > 1 ? roundProbability(1 / odd) : null;
}

function sumProbabilities(values: Array<number | null>): number | null {
  if (values.some((value) => value === null)) return null;
  return roundProbability(values.reduce<number>((sum, value) => sum + (value ?? 0), 0));
}

function compareRecommendations(left: QuickRecommendationDto, right: QuickRecommendationDto): number {
...
function isContradictory(left: MatchRecommendationMarket, right: MatchRecommendationMarket): boolean {
  const pair = new Set([left, right]);
  return (
    (pair.has("BTTS_YES") && pair.has("BTTS_NO")) ||
...
function riskFrom(dataQuality: number, confidence: number, warningCount: number): AnalysisRisk {
  if (dataQuality < 40 || confidence < 40 || warningCount > 2) return "VERY_HIGH";
  if (dataQuality < 60 || confidence < 60 || warningCount > 0) return "HIGH";
  if (confidence < 75) return "MEDIUM";
  return "LOW";
}

function riskRank(risk: AnalysisRisk): number {
...
function qualityLabel(score: number): AnalysisLabel {
...
function roundPercentagePoints(value: number): number {
