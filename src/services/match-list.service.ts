import {
  ...
  recordCacheError,
  setCacheEntry,
} from "../cache/cache-entry.js";

import {
  indexMatchList,
  matchListCache,
} from "../cache/match-list.cache.js";
...
import {
  parseMatchList,
} from "../parsers/match-list.parser.js";
...
import {
  loadAllMatches,
} from "./load-all-matches.service.js";
...
import {
  activeMatchLists,
  dedupe,
} from "./request-deduplication.service.js";
...
export async function getMatchList(
  ...
  if (entry && !force) {
    ...
    if (
      ...
      void refreshMatchList().catch(
        (error: unknown) => {
          console.error(
            "[matches] Background refresh failed:",
            errorMessage(error),
          );
        },
      );
  ...
  try {
    ...
    if (entry) {
      ...
      console.warn(
        "[matches] Refresh failed, returning previous cache:",
        message,
      );
...
export async function refreshMatchList():
  ...
  return dedupe(
    ...
    async () => {
      console.log(
        "[matches] Refresh started",
      );

      console.log(
        `[matches] Source URL: ${env.BETEXPLORER_BASE_URL}`,
      );
        ...
        await fetchSourceHtml(
          env.BETEXPLORER_BASE_URL,
          {
            ...
            onPage: async (
              ...
              console.log(
                "[matches] Browser page loaded, starting dynamic loading",
              );

              const loadingResult =
                await loadAllMatches(
                  page,
                  MATCH_LIST_LOADING_OPTIONS,
                );
              ...
              console.log(
                "[matches] Browser loading result:",
                {
                  initialCount:
                    loadingResult.initialCount,
                  ...
                  expandedLeagues:
                    loadingResult.expandedLeagues,

                  scrollPasses:
                    loadingResult.scrollPasses,

                  idlePasses:
                    loadingResult.idlePasses,

                  stopReason:
                    loadingResult.stopReason,

                  scrollTarget:
                    loadingResult.scrollTarget,
                },
              );
            },
          },
      ...
      if (!html.trim()) {
        throw new Error(
          "MATCH_LIST_EMPTY_HTML",
        );
      }

      console.log(
        `[matches] Final HTML size: ${html.length} characters`,
      );
      ...
      console.log(
        `[matches] Cheerio parsed ${data.matches.length} unique matches ` +
      ...
      if (
        ...
        console.warn(
          "[matches] Parser returned fewer matches than the browser:",
          {
            browserUniqueMatches:
              browserMatchCount,
      ...
      if (
        ...
        console.log(
          "[matches] Raw rows contain duplicate or alternative match rows:",
          {
            rawRows:
              browserRawRowCount,

            uniqueBrowserMatches:
              browserMatchCount,
      ...
      const previousCount =
        previousEntry?.data.matches
          .length ?? 0;

      validateMatchListRefresh(
        data,
      ...
      if (
        ...
        throw new Error(
          `MATCH_LIST_SUSPICIOUS_DROP:` +
            `${previousCount}->${data.matches.length}`,
        );
      }

      const validationDuration =
        elapsedMs(validationStarted);

      recordMatchListMetrics({
        lastInitialCount:
          browserMatchCount || data.matches.length,
        ...
        lastTotalDurationMs:
          ...
          validationDuration,
        matchesWithHomeLogo:
          data.matches.filter((match) => match.homeLogo).length,
        matchesWithAwayLogo:
          data.matches.filter((match) => match.awayLogo).length,
        matchesWithAnyOdds:
          data.matches.filter((match) => match.odds.available).length,
        matchesWithCompleteOdds:
        ...
        finishedMatches:
          data.matches.filter((match) => match.status === "FINISHED").length,
        finishedMatchesWithoutScore:
          data.matches.filter((match) =>
      ...
      console.log(
        `[matches] Cache updated successfully with ${data.matches.length} matches`,
      );
...
function validateMatchListRefresh(
  next: MatchListDto,
  previous: MatchListDto | null,
): void {
  if (!previous || previous.matches.length === 0) {
    return;
  }

  const previousHomeLogos =
    previous.matches.filter((match) => match.homeLogo).length;
  const previousAwayLogos =
    previous.matches.filter((match) => match.awayLogo).length;
  const previousOdds =
    previous.matches.filter((match) => match.odds.available).length;
  const previousFinishedScores =
    previous.matches.filter((match) =>
      match.status === "FINISHED" &&
  ...
  const nextHomeLogos =
    next.matches.filter((match) => match.homeLogo).length;
  const nextAwayLogos =
    next.matches.filter((match) => match.awayLogo).length;
  const nextOdds =
    next.matches.filter((match) => match.odds.available).length;
  const nextFinishedScores =
    next.matches.filter((match) =>
      match.status === "FINISHED" &&
      match.homeScore !== null &&
      match.awayScore !== null,
    ).length;
