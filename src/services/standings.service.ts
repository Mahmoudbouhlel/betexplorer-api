...
export async function getMatchStandings(
  ...
  if (entry && !force) {
    ...
    if (stale && backgroundRefresh) void refreshMatchStandings(eventId, options.url).catch(() => undefined);
    recordCacheHit();
  ...
  try {
    ...
    if (entry) return { data: entry.data, cache: createCacheMetadata(entry, cacheTtl.standings, false, true), warnings: [errorMessage(error)] };
    throw error;
  }
}

export async function refreshMatchStandings(eventId: string, url?: string): Promise<MatchStandingsDto> {
  validateEventId(eventId);
  ...
  return dedupe(activeStandings, eventId, async () => {
    ...
    if (data.groups.length === 0) {
      ...
      if (!ajaxUrl) throw new Error("STANDINGS_LINK_NOT_FOUND");
      data = parseStandings(await fetchSourceHtml(ajaxUrl), eventId, details);
    }
    recordParserMs("standings", elapsedMs(started));
    if (data.groups.length === 0) throw new Error("STANDINGS_PARSE_FAILED");
