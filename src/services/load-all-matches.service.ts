...
export async function getVisibleMatchEventIds(page: Page): Promise<string[]> {
...
export async function loadAllMatches(
  ...
  let stopReason: LoadAllMatchesResult["stopReason"] = "NO_NEW_MATCHES";
...
async function detectScrollTarget(page: Page): Promise<ScrollTarget> {
  return page.evaluate((candidates) => {
    for (const selector of candidates) {
      ...
      if (element.scrollHeight > element.clientHeight) {
        return { type: "ELEMENT" as const, selector };
      }
    }
...
async function getPageCounts(page: Page, rowSelector: string): Promise<PageCounts> {
  return page.evaluate((selector) => {
    const rows = Array.from(document.querySelectorAll(selector));
    const ids = rows.map((row) => row.getAttribute("data-event-id")).filter((id): id is string => Boolean(id));
...
async function getScrollMetrics(page: Page, target: ScrollTarget, bottomTolerancePx: number): Promise<ScrollMetrics> {
  return page.evaluate(
    ({ scrollTarget, tolerance }) => {
      const element =
...
async function scrollByTarget(page: Page, target: ScrollTarget, stepRatio: number, minimumStepPx: number): Promise<void> {
  await page.evaluate(
    ({ scrollTarget, ratio, minStep }) => {
      const element =
        scrollTarget.type === "ELEMENT"
          ? document.querySelector(scrollTarget.selector)
          : document.scrollingElement ?? document.documentElement;
...
async function scrollLastMatchIntoView(page: Page): Promise<void> {
  await page
...
async function waitForNewEventId(
  page: Page,
  rowSelector: string,
  previousSignature: string,
  timeoutMs: number,
): Promise<{ changed: boolean; ids: string[] }> {
  try {
    const ids = await page.waitForFunction(
      ...
      { selector: rowSelector, signature: previousSignature },
...
async function waitForStableSignature(
  page: Page,
  rowSelector: string,
  checks: number,
  intervalMs: number,
): Promise<{ stable: boolean; ids: string[] }> {
  let previous = "";
  let ids: string[] = [];
...
async function clickShowMore(page: Page): Promise<boolean> {
  return page.evaluate(() => {
    ...
    for (const candidate of candidates) {
      ...
      if (!/^(show more matches|show more|more matches|load more)$/i.test(text)) continue;
      if (candidate.closest("#H2HComponent,#bestOddsComponent,#lastResultsComponent,footer,[class*='advert'],[id*='advert']")) continue;
...
function eventIdSignature(ids: readonly string[]): string {
  return ids.slice().sort().join("|");
}

function emptyResult(stopReason: LoadAllMatchesResult["stopReason"], durationMs: number): LoadAllMatchesResult {
  return {
    initialCount: 0,
    finalCount: 0,
    rawRowCount: 0,
...
function buildResult(input: {
  ...
  stagnantIterations: number;
  reachedBottom: boolean;
  stopReason: LoadAllMatchesResult["stopReason"];
  durationMs: number;
  expandedLeagues: number;
  scrollTarget: ScrollTarget;
}): LoadAllMatchesResult {
  return {
