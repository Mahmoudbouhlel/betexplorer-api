...
export async function getMatchDetails(
  ...
  if (entry && !force) {
    ...
    if (stale && backgroundRefresh) void refreshMatchDetails(eventId, options.url).catch(() => undefined);
    recordCacheHit();
...
export async function refreshMatchDetails(eventId: string, url?: string): Promise<MatchDetailsDto> {
  validateEventId(eventId);
  ...
  return dedupe(activeDetails, eventId, async () => {
    ...
    if (!data.homeTeam.name || !data.awayTeam.name) throw new Error("MATCH_DETAILS_PARSE_FAILED");
    if (data.status === "UNKNOWN" && data.homeScore === null && data.awayScore === null) throw new Error("MATCH_DETAILS_SOURCE_NOT_READY");
...
export function resolveMatchUrl(eventId: string, url?: string): string {
  validateEventId(eventId);
  const resolved = url ?? getKnownMatchUrl(eventId);
  if (!resolved) throw new Error("MATCH_URL_REQUIRED");
  return resolved;
}

export async function waitForMatchDetailReady(page: Page): Promise<void> {
  try {
...
async function triggerRequiredLazySections(page: Page): Promise<void> {
...
async function waitForDetailSections(page: Page): Promise<void> {
  await page
    .waitForFunction(
      () =>
        Boolean(document.querySelector("#mainContent")) &&
        Boolean(
          document.querySelector("#bestOddsComponent") ||
            document.querySelector("#H2HComponent") ||
            document.querySelector("#standingsComponent"),
...
async function normalizeLazyImageSources(page: Page): Promise<void> {
  await page.evaluate(() => {
    const attributes = ["src", "data-src", "data-lazy-src", "data-original"];
    for (const image of Array.from(document.querySelectorAll("img"))) {
      for (const attribute of attributes) {
        const value = image.getAttribute(attribute);
        if (!value || value.startsWith("data:") || value.startsWith("javascript:") || value === "about:blank") continue;
