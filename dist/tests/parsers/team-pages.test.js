...
import { applyTeamResultsView, calculateTeamResultsSummary, parseBetExplorerTeamScore, parseTeamResultsPage, parseTeamResultsSource, sourceToResults, } from "../../src/parsers/team-results.parser.js";
import { parseTeamFixturesPage } from "../../src/parsers/team-fixtures.parser.js";
import { buildTeamFixturesUrl, buildTeamResultsUrl, extractTeamIdFromTeamUrl, parseTeamReferenceFromUrl, } from "../../src/utils/url.js";
const mocks = vi.hoisted(() => ({
    fetchSourceHtml: vi.fn(),
}));
...
describe("team page parser", async () => {
    ...
    it("parses half-time and second-half score", () => expect(parseBetExplorerTeamScore("2:1 (1:0, 1:1)")).toMatchObject({ halfTimeHome: 1, halfTimeAway: 0, secondHalfHome: 1, secondHalfAway: 1 }));
    ...
    it("missing table produces parser error", () => {
        expect(() => parseTeamResultsPage("<html><body><h1>Results & Fixtures</h1><p>Matches are listed below.</p></body></html>", reference)).toThrow("TEAM_RESULTS_TABLE_NOT_FOUND");
    });
...
describe("team page services", () => {
    ...
    it("full match team-results integration tolerates one team failure", async () => {
        ...
        mocks.fetchSourceHtml.mockImplementation(async (url) => {
            if (url.includes("netherlands-japan"))
                return matchWithTeamLinks();
            if (url.includes("/team/netherlands/"))
                return html;
            if (url.includes("/team/japan/"))
                throw new Error("TEAM_RESULTS_PARSE_FAILED");
            return html;
        });
...
//# sourceMappingURL=team-pages.test.js.map