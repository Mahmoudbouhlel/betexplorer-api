...
import { fullMatchCache } from "../cache/full-match.cache.js";
...
export async function getFullMatch(eventId, options = {}) {
    ...
    if (entry && !force && !needsMissingTeamResults) {
        ...
        if (stale && backgroundRefresh)
            void refreshFullMatch(eventId, options.url, options).catch(() => undefined);
...
export async function refreshFullMatch(eventId, url, options = {}) {
    validateEventId(eventId);
...
async function getFullTeamResults(eventId, url, limit) {
...
async function getTeamResultsForReference(reference, limit) {
...
//# sourceMappingURL=full-match.service.js.map