...
import { resolveTeamReference } from "./team-results.service.js";
export async function getTeamFixtures(teamId, options = {}) {
    ...
    if (entry && !force) {
        ...
        if (stale && backgroundRefresh)
            void refreshTeamFixtures(teamId, options).catch(() => undefined);
        recordCacheHit();
        return {
...
export async function refreshTeamFixtures(teamId, options = {}) {
    validateTeamId(teamId);
    const reference = resolveTeamReference(teamId, options);
    const key = teamFixturesCacheKey(teamId);
...
export function teamFixturesCacheKey(teamId) {
    return `fixtures:${validateTeamId(teamId)}`;
}
//# sourceMappingURL=team-fixtures.service.js.map