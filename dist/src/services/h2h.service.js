...
export async function getMatchH2H(eventId, options = {}) {
    ...
    if (entry && !force) {
        ...
        if (stale && backgroundRefresh)
            void refreshMatchH2H(eventId, options.url).catch(() => undefined);
        recordCacheHit();
    ...
    try {
        ...
        if (entry)
            return { data: entry.data, cache: createCacheMetadata(entry, cacheTtl.h2h, false, true), warnings: [errorMessage(error)] };
        throw error;
    }
}
export async function refreshMatchH2H(eventId, url) {
    validateEventId(eventId);
