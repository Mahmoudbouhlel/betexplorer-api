...
export const matchesRoutes = async (app) => {
    ...
    app.get("/api/matches/refresh-status", {
    ...
    app.get("/api/matches/:eventId", {
        ...
        if (!summary)
            return reply.code(404).send(apiError("MATCH_NOT_FOUND", "Match summary is not cached."));
        return summary;
    });
...
function routeOptions(tag, summary) {
    return {
        schema: {
            tags: [tag],
            summary,
...
function parseEventParams(request) {
    return eventIdParamSchema.parse(request.params);
}
function parseUrlQuery(request) {
...
function withOptionalUrl(query, options) {
    return query.url ? { ...options, url: query.url } : options;
}
function apiError(code, message) {
    return { error: { code, message } };
}
export function sendApiError(reply, status, code, message, details) {
