...
export function validateBetExplorerUrl(url) {
    let parsed;
    try {
        if (url.startsWith("//"))
...
export function parseTeamReferenceFromUrl(url) {
    let parsed;
    try {
        parsed = validateTeamUrl(url);
    }
    catch (error) {
        if (error instanceof Error && (error.message === "INVALID_TEAM_ID" || error.message === "INVALID_TEAM_SLUG"))
            throw error;
...
export function buildTeamResultsUrl(slug, teamId) {
    return `${buildTeamReference(slug, teamId).baseUrl}results/`;
}
export function buildTeamFixturesUrl(slug, teamId) {
    return `${buildTeamReference(slug, teamId).baseUrl}fixtures/`;
}
export function buildTeamReference(slug, teamId) {
...
export function absoluteBetExplorerUrl(value) {
    if (!value)
        return null;
    try {
...
export function relativeBetExplorerUrl(value) {
...
export function extractEventIdFromUrl(url) {
...
export function extractTeamIdFromTeamUrl(url) {
    const absolute = absoluteBetExplorerUrl(url);
    if (!absolute)
        return null;
    const segments = new URL(absolute).pathname.split("/").filter(Boolean);
    if (segments.length < 4 || segments[0] !== "football" || segments[1] !== "team")
        return null;
    const candidate = segments[3] ?? null;
    return candidate && TEAM_ID_PATTERN.test(candidate) ? candidate : null;
}
export function extractTeamSlugFromTeamUrl(url) {
    const absolute = absoluteBetExplorerUrl(url);
    if (!absolute)
        return null;
    const segments = new URL(absolute).pathname.split("/").filter(Boolean);
...
function validateTeamUrl(url) {
    ...
    try {
        ...
        if (error instanceof Error && error.message === "INVALID_TEAM_ID")
            throw error;
        if (error instanceof Error && error.message === "INVALID_TEAM_SLUG")
            throw error;
        throw new Error("INVALID_TEAM_URL", { cause: error });
    }
    return parsed;
